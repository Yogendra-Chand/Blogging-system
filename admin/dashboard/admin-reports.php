                ...
                $sql = "SELECT 
                        ...
                        INNER JOIN posts p ON p.post_id = r.post_id
                ...
                <body class="sb-nav-fixed">
                    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
                        <!-- Navbar Brand-->
                        <a class="navbar-brand ps-3" href="index.php">The Daily Blogs</a>
                        <!-- Sidebar Toggle-->
                ...
                <?php
                $authorQuery = mysqli_query($conn, "SELECT username FROM users WHERE user_id = '{$row['author_id']}'");
                $authorRow = mysqli_fetch_assoc($authorQuery);
                ?>
                <td><?= htmlspecialchars($authorRow['username']) ?></td>

                    <td><?= htmlspecialchars($row['title']) ?></td>
                    <td><?= htmlspecialchars($row['reason']) ?></td>

                    <!-- this is to display the creation date -->
                    <td><?= $row['created_at'] ?></td>
                    <td>
                <a href="view-reported-blog.php?post_id=<?= $row['post_id'] ?>&reason=<?= urlencode($row['reason']) ?>" 
                class="btn btn-info btn-sm">
                View
                </a>

                <a href="notify_author.php?post_id=<?= $row['post_id']; ?>&user_id=<?= $row['author_id']; ?>"
                class="btn btn-sm btn-warning">
                Notify Author
                </a>

                <a href="../../delete-blog.php?id=<?= $row['post_id']; ?>"
            ...
            <footer class="py-4 bg-light mt-auto">
                <div class="container-fluid px-4">
                    <div class="d-flex align-items-center justify-content-between small">
                        <div class="text-muted">Copyright &copy; The Daily Blogs 2026</div>
                        <div>
